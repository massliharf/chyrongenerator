import { Type, Palette, Layout, Download, ChevronDown, ChevronRight, Play, Clapperboard, Video, Sliders, X } from 'lucide-react';
...
const SliderControl = ({ label, value, onChange, min, max, step, unit = '' }: { label: string, value: number, onChange: (v: number) => void, min: number, max: number, step: number, unit?: string }) => (
    <div className="space-y-2.5">
        ...
        <input type="range" min={min} max={max} step={step} value={value} onChange={(e) => onChange(parseFloat(e.target.value))} className="clean-slider" />
    </div>
);

// New Composite Color Picker Component
const ColorPickerComposite = ({ label, value, onChange }: { label: string, value: string, onChange: (v: string) => void }) => (
    <div className="w-full">
        <label className="block text-[11px] font-semibold text-gray-500 mb-1.5 uppercase tracking-wider">{label}</label>
        <label className="composite-color-picker">
            <input type="color" value={value} onChange={(e) => onChange(e.target.value)} className="w-8 h-8 rounded-l-md clean-color border-r border-gray-200 m-[-1px]" />
            <span className="text-[12px] font-mono text-gray-600 uppercase flex-1 text-center py-1.5 font-medium">{value}</span>
        </label>
    </div>
);
...
export function Sidebar({
    ...
    const inputClasses = "w-full bg-white border border-gray-300 rounded-md px-3 py-2 text-[13px] focus:ring-2 focus:ring-blue-100 focus:border-blue-500 outline-none shadow-sm placeholder-gray-400 text-gray-900 transition-all font-medium appearance-none";
    const labelClasses = "block text-[11px] font-semibold text-gray-500 mb-1.5 uppercase tracking-wider";

    return (
        <div className="w-full md:w-[380px] bg-white border-r border-gray-200 p-5 flex flex-col h-full overflow-y-auto clean-scrollbar select-none relative pb-32">
            ...
            <div className="space-y-0 flex-grow">
                ...
                <ControlGroup title="Presets" icon={Layout} defaultOpen={false}>
                    <div className="space-y-3">
                        <div className="flex gap-2">
                            <input type="text" value={newPresetName} onChange={(e) => setNewPresetName(e.target.value)} placeholder="Save current style as..." className={inputClasses} />
                            <button onClick={() => { if (newPresetName.trim()) { onSavePreset(newPresetName); setNewPresetName(''); } }} disabled={!newPresetName.trim()} className="px-3 py-2 bg-white text-gray-700 border border-gray-300 rounded-md text-[13px] font-medium hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm transition-colors whitespace-nowrap">Save</button>
                        </div>

                        {presets.length > 0 && (
                            <div className="flex overflow-x-auto pb-2 -mx-1 px-1 clean-scrollbar space-x-2 w-full">
                                {presets.map((preset) => (
                                    <div key={preset.id} className="preset-chip group relative">
                                        <button onClick={() => onLoadPreset(preset)} className="focus:outline-none">{preset.name}</button>
                                        <button onClick={(e) => { e.stopPropagation(); onDeletePreset(preset.id); }} className="w-4 h-4 ml-1 rounded-full bg-gray-100 text-gray-400 flex items-center justify-center hover:bg-red-100 hover:text-red-500 transition-colors focus:outline-none" title="Delete preset">
                        ...
                        {presets.length === 0 && (
                            <div className="text-[12px] text-gray-400 text-center py-3 bg-gray-50 border border-dashed border-gray-200 rounded-md font-medium">No saved presets</div>
                        )}
                    </div>
                </ControlGroup>
                ...
                <ControlGroup title="Content" icon={Type} defaultOpen={true}>
                    <div className="space-y-4">
                        <div>
                            ...
                            <textarea value={text} onChange={(e) => setText(e.target.value)} className={`${inputClasses} min-h-[90px] resize-y font-bold`} placeholder="Enter text..." />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input type="text" value={subtitle} onChange={(e) => setSubtitle(e.target.value)} className={inputClasses} placeholder="Subtitle text..." />
                        </div>
                    </div>
                </ControlGroup>

                <div className="py-2 border-b border-gray-200">
                    <button onClick={() => setShowAdvanced(!showAdvanced)} className="w-full py-2.5 text-gray-500 hover:text-gray-800 font-semibold text-[11px] uppercase tracking-wider flex items-center justify-center gap-1.5 transition-colors bg-gray-50 hover:bg-gray-100 rounded-lg">
                ...
                {showAdvanced && (
                    <div className="space-y-0 animate-in fade-in slide-in-from-top-4 duration-300">
                        ...
                        <ControlGroup title="Themes & Colors" icon={Palette} defaultOpen={true}>
                            <div className="space-y-5">
                                <div className="grid grid-cols-2 gap-x-4 gap-y-4">
                                    <ColorPickerComposite label="Main Tile" value={tileColor} onChange={setTileColor} />
                                    <ColorPickerComposite label="Main Text" value={textColor} onChange={setTextColor} />
                                    <ColorPickerComposite label="Sub Banner" value={subTileColor} onChange={setSubTileColor} />
                                    <ColorPickerComposite label="Sub Text" value={subTextColor} onChange={setSubTextColor} />
                                </div>
                                ...
                                <div className="p-3 bg-gray-50 border border-gray-200 rounded-md space-y-4">
                                    <div className="flex items-center justify-between">
                                        <div>
                                            ...
                                            <p className="text-[10px] text-gray-500">Backdrop color for testing</p>
                                        </div>
                                    ...
                                    <div className="flex items-center justify-between">
                                        <div>
                                            ...
                                            <p className="text-[10px] text-gray-500">Adds blurred shadow plane</p>
                                        </div>
                        ...
                        {/* SUBTITLE SETTINGS */}
                        <ControlGroup title="Subtitle Adjustments" icon={Type} defaultOpen={false}>
                            <div className="space-y-5">
                                <div>
                                    <label className={labelClasses}>Alignment</label>
                                    <div className="grid grid-cols-2 gap-1 bg-gray-100 p-1 rounded-md">
                                        <button onClick={() => setSubtitlePos('top')} className={`px-3 py-1.5 rounded-[4px] text-[12px] font-medium transition-all ${subtitlePos === 'top' ? 'bg-white text-gray-900 shadow-sm border border-gray-200' : 'text-gray-500 hover:text-gray-700'}`}>Top</button>
                                        <button onClick={() => setSubtitlePos('bottom')} className={`px-3 py-1.5 rounded-[4px] text-[12px] font-medium transition-all ${subtitlePos === 'bottom' ? 'bg-white text-gray-900 shadow-sm border border-gray-200' : 'text-gray-500 hover:text-gray-700'}`}>Bottom</button>
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <SliderControl label="Pad (X)" value={subtitlePadding.x} onChange={(v) => setSubtitlePadding({ ...subtitlePadding, x: v })} min={0} max={80} step={2} unit="px" />
                                    <SliderControl label="Pad (Y)" value={subtitlePadding.y} onChange={(v) => setSubtitlePadding({ ...subtitlePadding, y: v })} min={0} max={60} step={2} unit="px" />
                                </div>
                            </div>
                        </ControlGroup>
                        ...
                        <ControlGroup title="Dimensions" icon={Layout}>
                            <div className="space-y-5">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <SliderControl label="Kerning" value={tileGap} onChange={setTileGap} min={0} max={60} step={1} unit="px" />
                        ...
                        <ControlGroup title="Organic Chaos" icon={Sliders}>
                            <div className="space-y-6">
                                <div className="space-y-4">
                                    <h4 className={labelClasses}>Shadow Parameters</h4>
                                    <div className="grid grid-cols-2 gap-4">
                                        <SliderControl label="Depth Offset" value={shadowOffset} onChange={setShadowOffset} min={0} max={40} step={1} unit="px" />
                                        <SliderControl label="Variation" value={shadowChaos} onChange={setShadowChaos} min={0} max={10} step={0.5} />
                                    </div>
                                </div>
                                <div className="space-y-4 pt-2 border-t border-gray-100">
                                    <h4 className={labelClasses}>Transform Chaos</h4>
                                    <SliderControl label="Rotation Tilt" value={chaosLevel} onChange={setChaosLevel} min={0} max={15} step={0.5} unit="°" />
                                    <div className="grid grid-cols-2 gap-4">
                                        <SliderControl label="Pos Jitter" value={posChaos} onChange={setPosChaos} min={0} max={20} step={0.5} unit="px" />
                                        <SliderControl label="Scale Jitter" value={scaleChaos} onChange={setScaleChaos} min={0} max={1} step={0.05} />
                                    </div>
                                </div>
                            </div>
                        </ControlGroup>
                        ...
                        {/* ANIMATION */}
                        <ControlGroup title="Animation Setup" icon={Clapperboard} defaultOpen={false}>
                            <div className="space-y-5">
                                <div>
            ...
            <div className="absolute bottom-0 left-0 right-0 p-5 bg-white border-t border-gray-200 z-10 space-y-2">
                ...
                {animationPreset !== 'none' && (
                    <div className="mb-2">
                        <button
                            ...
                            className="w-full px-4 py-2.5 bg-gray-900 hover:bg-black text-white rounded-md text-[13px] font-bold shadow-sm transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            {isExportingVideo ? (
                                <>
